...
actor class UserIndex() = this {
  ...
  public shared({ caller }) func registerUser(uid: T.UID, form: T.RegisterForm, beneficiary: ?T.BID) : async() {
    ...
    let responseJson = await HTTP.canister.post({
      ...
      let { token; beneficiaryId }: StoreUserResponse = switch(Serde.JSON.fromText(responseJson, null)) {
        case(#err(_)) throw Error.reject("cannot serialize storeResponse data");
        case(#ok(blob)) {
          let response: ?StoreUserResponse = from_candid(blob);

          switch(response) {
            case(null) throw Error.reject("cannot serialize storeResponse data");
            case(?value) value;
          };
        };
      };
    ...
    // WARN just for debug
    Debug.print("token generated by user " # Principal.toText(uid) #" --> " # token);
