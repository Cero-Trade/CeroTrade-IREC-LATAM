    ...
    public func isSellingToken(user : T.UID, tokenId : T.TokenId) : async Bool {
        switch(tokensInMarket.get(tokenId)) {
            ...
            case (?tokenMarketInfo) {
                // verify is user is already selling the token
                for (userMarketInfo in tokenMarketInfo.usersxToken.vals()) {
                    if (userMarketInfo.UID == user) {
                    return true;
                    }
                };
                return false;
    ...
    public func getUserTokensOnSale(user : T.UID, tokenId : T.TokenId) : async Nat {
        switch (tokensInMarket.get(tokenId)) {
            ...
            case (?tokenMarketInfo) {
                for (userMarketInfo in tokenMarketInfo.usersxToken.vals()) {
                    if (userMarketInfo.UID == user) {
                    return userMarketInfo.quantity;
                    }
                };
            };
        };
